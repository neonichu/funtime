...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self.modelObject class])
                                                            forIndexPath:indexPath];
    
    cell.textLabel.text = [NSString stringWithCString:property_getName(properties[indexPath.row])
                                             encoding:NSUTF8StringEncoding];
    
    UILabel* detailLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, 220.0, 44.0)];
